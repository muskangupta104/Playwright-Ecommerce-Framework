...
- Name: login.spec.js >> Verify user can add product to cart successfully
...
```ts
  ...
  3  | const AddToCart = require("../pages/AddToCart");
  ...
  10 |     await login.login(data.username, data.password);
  11 | });
  12 | 
  13 | 
  14 | test("Verify user can add product to cart successfully", async ({ page }) => {
  15 |     const cart = new AddToCart(page);
  16 | 
  17 |     await cart.product.click();
  18 | 
> 19 |     await expect(cart.cartBadge).toHaveText("1");
     |                                  ^ Error: toHaveText can be only used with Locator object, was called with undefined
  20 | });
```
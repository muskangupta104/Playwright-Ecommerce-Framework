...
- Name: login.spec.js >> Verify user can login successfully
- Location: tests\login.spec.js:8:1
...
```ts
  ...
  5  | const data = require("../utils/testData");
  6  | 
  7  | 
  8  | test("Verify user can login successfully", async ({ page }) => {
  9  | 
  ...
  20 |    "https://www.saucedemo.com/inventory.html"
  21 | 
  22 |  );
> 23 | test("Verify user can add the product to cart successfully", async ({ page }) => {
     |  ^ Error: Playwright Test did not expect test() to be called here.
  24 | 
  25 |   const cart = new AddToCart(page);
  26 | 
  27 | await cart.product.click();
  28 | })
  29 | });
```